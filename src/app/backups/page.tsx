...
export default function BackupsPage() {
  ...
  const fetchPlans = useCallback((p: number) => {
    ...
    getBackupPlansHistory(p)
      ...
      .catch(() => setFetchError("Failed to load backup plans"))
  ...
  useEffect(() => {
    ...
    getBackupPlansHistory(page)
      .then((data) => { if (!cancelled) { setFetchError(null); setPlans(data.items); setTotalPages(data.total_pages); } })
      .catch(() => { if (!cancelled) setFetchError("Failed to load backup plans"); })
  ...
  const handleGenerate = async () => {
    ...
    setGenError(null);
    try {
      await generateBackupPlan({ career_id: selectedCareer.id, max_scenarios: 3, progress_token: token });
      setModalOpen(false);
      ...
      <div className="mb-8 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={() => setModalOpen(true)}>
            <Sparkles className="mr-1 h-4 w-4" /> {plans.length > 0 ? "Regenerate Backup Plan" : "Generate Backup Plan"}
          </Button>
        </div>
      </div>
          ...
          {generating && (
            <div className="rounded-lg border border-accent/30 bg-accent/5 p-4">
              <GenerationProgress
                ...
                message={progress?.message ?? "Preparing your backup plan"}
